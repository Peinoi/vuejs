...
// 실제 라우팅 등록 영역
//전체 조회 : URI = /boards, METHOD = GET
router.get(`/boards`, async (req, res) => {
  let list = await boardService.findAll().catch((err) => {
    console.log(err);
  });
  res.send(list);
});
...
router.get(`/boards/:bno`, async (req, res) => {
  ...
  let info = await boardService.findByNo(no).catch((err) => {
    console.log(err);
  });
  res.send(info);
});
...
router.post(`/boards`, async (req, res) => {
  ...
  let result = await boardService.createBoard(boardInfo).catch((err) => {
    console.log(err);
  });
  res.send(result);
});
...
router.put(`/boards/:bno`, async (req, res) => {
  let boardInfo = req.body;
  let no = req.params.bno;
  let result = await boardService.boardUpdate(no, boardInfo).catch((err) => {
    console.log(err);
  });
  res.send(result);
});
//삭제 URI = /boards METHOD = DELETE
